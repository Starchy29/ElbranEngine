...
void GraphicsAPI::ApplyPostProcesses(DrawComponents app, const PostProcess* postProcessSequence, uint8_t ppCount) {
	for(uint32_t i = 0; i < ppCount; i++) {
		...
		postProcessSequence[i].Render(input, output, app);
	}
...
void GraphicsAPI::SetVertexShader(const VertexShader* shader, const void* constantInput, uint32_t inputBytes) const {
	if(constantInput) {
...
void GraphicsAPI::SetGeometryShader(const GeometryShader* shader, const void* constantInput, uint32_t inputBytes) const {
	if(constantInput) {
		WriteBuffer(constantInput, inputBytes, shader->constants);
...
void GraphicsAPI::SetPixelShader(const PixelShader* shader, const void* constantInput, uint32_t inputBytes) const {
	if(constantInput) {
		WriteBuffer(constantInput, inputBytes, shader->constants);
...
void GraphicsAPI::CreateDefaultInputLayout(LoadedFile vertexShaderBlob) const { platformGraphics->CreateDefaultInputLayout(vertexShaderBlob); }
...
void GraphicsAPI::SetBlendMode(BlendState mode) const { platformGraphics->SetBlendMode(mode); }
void GraphicsAPI::SetPrimitive(RenderPrimitive primitive) const { platformGraphics->SetPrimitive(primitive); }
...
void GraphicsAPI::SetComputeTexture(const ComputeTexture* texture, uint8_t slot) const { platformGraphics->SetComputeTexture(texture, slot); }
void GraphicsAPI::SetEditBuffer(const EditBuffer* buffer, uint8_t slot) const { platformGraphics->SetEditBuffer(buffer, slot); }
void GraphicsAPI::SetOutputBuffer(const OutputBuffer* buffer, uint8_t slot, const void* initialData) const { platformGraphics->SetOutputBuffer(buffer, slot, initialData); }
...
void GraphicsAPI::SetConstants(ShaderStage stage, GraphicsBuffer* buffer, uint8_t slot) const { platformGraphics->SetConstants(stage, buffer, slot); }
void GraphicsAPI::SetArray(ShaderStage stage, const ArrayBuffer* buffer, uint8_t slot) const { platformGraphics->SetArray(stage, buffer, slot); }
void GraphicsAPI::SetTexture(ShaderStage stage, const Texture2D* texture, uint8_t slot) const { platformGraphics->SetTexture(stage, texture, slot); }
void GraphicsAPI::SetTextureArray(ShaderStage stage, const Texture2DArray* textures, uint8_t slot) const { platformGraphics->SetTextureArray(stage, textures, slot); }
void GraphicsAPI::SetSampler(ShaderStage stage, Sampler* sampler, uint8_t slot) const { platformGraphics->SetSampler(stage, sampler, slot); }
...
void GraphicsAPI::ClearMesh() const { platformGraphics->ClearMesh(); }
